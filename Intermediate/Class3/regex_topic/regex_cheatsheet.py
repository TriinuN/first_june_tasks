...
patternText = re.compile(r'abcdefghijklmno')    # To find exact text
...
patternOptionalCharacters = re.compile(r'ab?c') # Optional character
patternExactNumber = re.compile(r'\+(\d{3}).(\d{3}).(\d{2}.\d{2})')
patternRangeOfNumber = re.compile(r'\+(\d{3}).(\d{3}).(\d{2,4})')

# result = re.findall(patternOptionalCharacters, play_string)
# print(result)
...
# Split functionality
resultSplit = patternText.split(play_string)
print(resultSplit)
